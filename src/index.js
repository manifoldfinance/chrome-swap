...
const provider = ethProvider('ws://127.0.0.1:1248?identity=web3connect')
...
chrome.runtime.onMessage.addListener((payload, sender, sendResponse) => {
  if (payload.method === 'wc_sessionRequest') return provider.connection.send(payload)
  const id = provider.nextId++
  pending[id] = { tabId: sender.tab.id, payloadId: payload.id, method: payload.method }
  const load = Object.assign({}, payload, { id, __web3connectOrigin: getOrigin(sender.url) })
...
//     const load = { jsonrpc: '2.0', id: 1, method: 'wc_sessionRequest', params: [] }
//     provider.connection.send(load)
//   } else {
//     if (provider && provider.close) provider.close()
//     provider = ethProvider('ws://127.0.0.1:1248?identity=web3connect')
//   }
// })
